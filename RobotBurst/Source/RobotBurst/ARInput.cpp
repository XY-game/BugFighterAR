...
void UARInput::UpdateDetection(float DeltaTime)
{
	UpdateGameKeys(DeltaTime);
	ProcessKeyStates(DeltaTime);
}

FVector2D UARInput::GetTouchAnchor(int32 i) const
{
	return (i >= 0 && i < ARRAY_COUNT(TouchAnchors)) ? TouchAnchors[i] : FVector2D::ZeroVector;
}

void UARInput::UpdateGameKeys(float DeltaTime)
{
...
void UARInput::ProcessKeyStates(float DeltaTime)
{
	AARPlayerController* MyController = CastChecked<AARPlayerController>(GetOuter());
...
void UARInput::DetectOnePointActions(bool bCurrentState, bool bPrevState, float DeltaTime, const FVector2D & CurrentPosition, FVector2D & AnchorPosition, float & DownTime)
{
...
void UARInput::DetectTwoPointsActions(bool bCurrentState, bool bPrevState, float DeltaTime, const FVector2D & CurrentPosition1, const FVector2D & CurrentPosition2)
{
